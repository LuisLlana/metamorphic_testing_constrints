...
planifica las tareas de un proyecto, con el objetivo de minimizar el tiempo de ejecución del proyecto y, respetando una combinación de restricciones de precedencia y de recursos. Las restricciones de precedencia entre dos tareas establece que una tarea no puede comenzar hasta que la otra haya terminado, y las restricciones de recursos limitan la cantidad de recursos utilizados por las tareas que se ejecutan al mismo tiempo.
...
\section{Proposal}
Nuestra propuesta pretende probar los programas en MiniZinc (lenguaje de lógica de restricciones) que implementan problemas de planificación, por seguir un esquema fijo y conocido. Para ello, se propone, por un lado, utilizar la prueba metamórfica, incluyendo el diseño e implementación de relaciones metamórficas en el contexto de estos programas, y aplicarlas para obtener un mejor conjunto de casos de prueba, que el original dado. Así mimso, se propone, aplicar la prueba de mutaciones, que al no existir un sistema de mutación para este lenguaje de programación, se pretende diseñar los opradores de mutación apropiados, implem,entar y aplicar, para obtener mutantes qeu nos permitan probar el conjunto de casos de prueba. Posteriormente, evaluaremos tanto las relaciones metamórficas como los operadores de mutación.
...
Falta definir las relaciones metamórficas: MR1, MR2, MR3, etc.
...
Para evaluar la propuesta planteada, que combina las dos técnicas de prueba de software descritas (la prueba de mutaciones y la prueba metamórfica), se plantean en esta sección varias preguntas de investigación, y se describe el programa sobre el que se realizarán los experimentos.
...
\paragraph{\rqmetamorphic} \emph{¿Cómo mejora la cobertura de defectos combinar las pruebas de mutación la prueba metamórfica? ¿En qué medida? ¿Cuánto mejora el conjunto de casos de prueba antes y después de aplicar las relaciones metamórficas?}

Puesto que la prueba metamórfica se aplica para mejorar la cobertura de mutación, el objetivo es obtener nuevos casos de prueba que maten mutantes, cuyos casos iniciales no mataban mutantes, obteniendo un conjunto de casos de prueba más completo y por tanto, de mejor calidad. Por tanto, para responder a esta cuestión es necesario medir la mejora que aporta la prueba metamórfica al conjunto inicial

\paragraph{\rqidentifying} \emph{¿Existen características comunes en las relaciones metamórficas más efectivas para programas con restricciones, especialmente para programas con restricciones que especifican problemas de scheduling?}
...
Se han aplicado 33 de los 43 operadores de mutación anteriormente definidos en la tabla~\ref{tab:Op-MiniZinc} al modelo de programa elegido (\texttt{rcpsp.mzn}), generándose un total 76 mutanes. Tras la ejecución del conjunto de datos iniciales procedentes del repositorio....¿cuántos casos de prueba?, se han matado 68 mutantes, quedando vivos 8, que tras analizarlos, 2 de ellos han resultado ser equivalentes, como se muestra en la tabla~\ref{tab:mutant-counts}. Por lo que la puntuación de mutación en la primera ronda es del 91.8\% . 

\textbf{¿Qué operadores de mutación son más efectivos para detectar defectos en lenguajes con restricciones?}
Respecto a  los 33 operadores de mutación aplicados, los mutantes que quedan vivos en esta primera ronda, son generados por los operadores: C2D, EQ2LE, GT2NE, LT2GT y NE2GT. De ellos uno es de expresiones lógicas y el resto de expresiones relacionales. Se puede observar que los operadores específicos del lenguaje MiniZinc aplicados: CSWAP(intercambio de los parámetros 2 y 3 de \texttt{cumulative}) y F2E(sustituye la aparición de un cuantificador lógico \texttt{forall} por un cuantificador  \texttt{exists}), han generado mutantes que han sido todos matados por el conjunto de casos de prueba inicial.	Por lo que se deduce que son efectivos en detectar errores en este tipo de programas.
