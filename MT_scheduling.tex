...
%%% TODO CArmina. Revisa Antonio inglés 
%%% Introducir la importancia de los problemas de scheduling y su uso en diferentes disciplinas y contextos: convendría tener citas a aplicaciones de ejemplo (fabricación, logística, gestión de proyectos, etc.)
\begin{comment}
    

The Resource Constrained Project Scheduling Problem (RCPSP) is an NP-Hard combinatorial problem
that attracts the attention of researchers due to its computational
complexity \cite{herroelen1998resource, abdolshah2014review,hartmann2022updated}. 
The main difficulty when dealing with NP-hard problems is the exponential growth of the computational resources needed to solve them.

The RCPSP consists of planning a set of tasks that have a fixed duration and cannot be interrupted. Among the tasks, there are precedence constraints that force some tasks to start after the completion of others.
In addition, processing each task requires certain resources, which are available in a limited amount. Therefore, tasks must be planned taking into account resource constraints.
...
In order to measure the quality of test suite with this technique, the \textit{mutation score} is often used. This widely used measure is defined as the number of killed mutants divided by the total number of mutants generated (total mutants minus equivalent mutants), expressed as a percentage:
\begin{displaymath}
  MS = K / (G-E)
\end{displaymath}
...
% TODO (Carmina): introducir mutation score como una métrica común para evaluar conjuntos de
% prueba utilizando mutation testing
...
% TODO: ver si se puede reducir el espacio entre la fórmula y el marco en el entorno
\begin{framed}
  \begin{displaymath}
      \begin{array}{l}
...
%DONE: programa bajo prueba? Sonia y Luis
...
% TODO explicar qué queremos decir con mutation-effective
...
% DONE: añadir defensa de que con las MRs existentes ya se han matado todos los mutantes no equivalentes
...
% TODO: Carmina para escribir la primera versión, Antonio revisa
